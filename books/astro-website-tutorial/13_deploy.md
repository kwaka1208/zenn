...
このチュートリアルで作るサイトを、実際にGitHub Pagesで公開しています。「自分のサイトはこう表示されるはずだ」という答え合わせに使ってください。

https://kwaka1208.github.io/my-astro-site/

ソースコード一式も公開しています。うまく動かない箇所があるときは、自分のファイルと見比べてみてください。
...
以下、リポジトリ名を `my-astro-site` として説明します。自分のリポジトリ名に読み替えてください。前節で紹介した見本サイトもこの方法で公開しているので、設定を入れたあとのファイルがどうなるかは、見本のソースコードでも確認できます。
