...
このファイルには新しい仕組みが2つ入っています。順番に見ていきましょう。
...
なぜでしょうか。スコープ付きスタイルは「このファイルに書かれたHTMLタグ」だけに効く仕組みです。ところが記事本文の `<h2>` は、Markdownから**変換されて生まれたHTML**であり、このファイルに直接書かれたタグではないため、スコープの対象外なのです。
