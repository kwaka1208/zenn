...
3. メニューの「追加機能」から「イベント登録」を選んでください（「追加機能」が表示されるまでに時間がかかることがあります。表示されていない場合1〜2分程度待ってみてください）。
...
登録したイベントを削除する場合も、操作の流れは同じです。スプレッドシートで削除したい行にチェックを入れ、メニューの「追加機能」から「イベント削除」を選んでください。

すでに実施済みのイベントを残したい場合は、スプレッドシートからその日付の行を外してから実行してください。

### 不具合の報告などはこちらへ
こちらのツールに不具合がございましたら、[こちら](https://github.com/kwaka1208/issues/issues)から報告いただけると助かります。
